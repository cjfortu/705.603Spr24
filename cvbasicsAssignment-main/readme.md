...
An image classifier webservice, with optional code to conduct a degradation point study.  

***The original assignment instructions, with notebook details, are included at the end of this readme.***  

## Running the Webservice
To run the model and services, you can build a docker image from *Dockerfile* and run the container.  For a preview simply run all cells in *webservice_demo.ipynb*.  
* An HTTP get request can be sent to http://localhost:8786/post. The body must include the desired candidate image.  
* The image classification results can be seen at http://localhost:8786/predict.  

To run the degradation point study, simply run all cells in *ObjectDetection.ipynb*.  
